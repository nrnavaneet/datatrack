...
6. **`pytest -q tests/`** for the unit suite (includes a `python -m datatrack --help` smoke test so module entrypoints stay healthy). Discovery defaults live in `pyproject.toml` under `[tool.pytest.ini_options]`.
